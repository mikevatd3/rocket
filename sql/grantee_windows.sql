-- Grantee maximum window (>=3 in a year)
...
SELECT DISTINCT ON (grantee) grantee, sale_date, transaction_within_year
FROM transaction_window_counts
WHERE transaction_within_year >= 3
ORDER BY grantee, transaction_within_year DESC, sale_date ASC;
